...
public class Server {
	...
	public void init() {  
		...
		try {  
			...
			while (true) { //服务器端要不断接收来自客户端的所有请求，要一直监听 
				Socket client = serverSocket.accept();  
				//一个客户端连接就开户两个线程处理读写  
				new Thread(new ReadHandlerThread(client)).start();  //读
				new Thread(new WriteHandlerThread(client)).start(); //写
			}  
		} catch (Exception e) {  
			e.printStackTrace();  
		} finally{
	...
	public void run() {
		...
		try{
			while(true){
				...
				System.out.println("客户端:" + reciver); 
			}
		}catch(Exception e){
			e.printStackTrace();
		}finally{
